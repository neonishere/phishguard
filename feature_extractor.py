...
Extracts 29 lexical features from a URL string.
No external HTTP requests are made - all features are derived
from the URL string itself, keeping inference fast.
"""
...
def normalise_url(url):
    """Add http:// prefix if the URL has no scheme."""
    url = str(url).strip()
...
def extract_features(url):
    ...
    try:
        ...
        return {
            ...
            'special_char_ratio':  round(sum(not c.isalnum() and c not in '.-_/' for c in url) / max(len(url), 1), 4),
            'longest_word_len':    max((len(w) for w in re.split(r'[.\-_/]', hostname) if w), default=0),
